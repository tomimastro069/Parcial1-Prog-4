...
export default function MisPedidosPage() {
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <p className="eyebrow mb-2">Tu historial</p>
      <h1 className="font-baskerville text-3xl font-bold text-coffee mb-6 tracking-[-0.01em]">Mis pedidos</h1>
        ...
        {isLoading ? (
          ...
          <div className="bg-cream-soft border border-line p-8 rounded-lg shadow-sm text-center">
            <p className="text-clay">No realizaste ningún pedido todavía.</p>
          </div>
        ) : (
          <div className="space-y-6">
            {data.items.map((pedido, index) => (
              <div
                key={pedido.id}
                className="bg-cream-soft rounded-lg shadow-sm border border-line overflow-hidden"
                style={{ animation: 'slideUp 0.3s ease-out both', animationDelay: `${index * 0.06}s` }}
              >
                <div className="bg-cream px-6 py-4 border-b border-line flex flex-wrap justify-between items-center gap-4">
                  <div>
                    <span className="text-sm text-sand">Pedido #{pedido.id}</span>
                  ...
                  <div className="flex items-center gap-3">
                    <div className="text-right">
                      <span className="text-sm text-sand">Total</span>
                      <p className="font-baskerville text-lg font-bold text-walnut">${pedido.total.toFixed(2)}</p>
                    </div>
                ...
                <div className="p-6 grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div>
                    <h4 className="text-sm font-semibold text-bark mb-2">Detalles</h4>
                  ...
                  <div>
                    <h4 className="text-sm font-semibold text-bark mb-2">Resumen</h4>
